    ...
    <div v-for="cartItem in cartItems" :key="cartItem.product.id" class="row mt-2 pt-3 justify-content-around">
      ...
      <div class="col-md-5 px-3">
        <div class="card-block px-3">
          ...
          <p id="item-total-price" class="mb-0">Total : <span class="font-weight-bold"> $ {{ (cartItem.product.price*cartItem.quantity).toFixed(2) }}</span></p>
...
export default {
  ...
  methods: {
    ...
    deleteItem(cartItem) {
      ...
      axios.delete(`${this.baseURL}cart/delete?cartItemId=${cartItem}&token=${this.token}`)
        .then((response)=>{
          if(response.status == 200){
